...
import { Plus, Gift as GiftIcon, Edit3, Trash2, Wand2, X, Upload, RefreshCw, AlertCircle, Video, Clock, Image as ImageIcon, Maximize, PlayCircle, Settings2, Save, Layers, ShieldCheck } from 'lucide-react';
  ...
  const renderPreview = (url: string) => {
    ...
    return (
      <div className="relative w-full h-full">
        {isVideoUrl(url) ? (
          <video src={url} autoPlay muted loop className="w-full h-full object-contain rounded-2xl pointer-events-none" />
        ) : (
          <img src={url} className="w-full h-full object-contain rounded-2xl pointer-events-none" alt="Preview" />
        )}
        <div className="absolute inset-0 bg-transparent z-10"></div> {/* طبقة حماية تمنع الحفظ */}
      </div>
    );
  };
        ...
        {gifts.map(gift => (
          <div key={gift.id} className="bg-slate-950/60 p-4 rounded-[2rem] border border-white/10 flex flex-col items-center gap-2 group relative">
            ...
            <div className="w-16 h-16 flex items-center justify-center mb-1 relative">
               ...
               {gift.catalogIcon ? (
                 <img src={gift.catalogIcon} className="w-full h-full object-contain pointer-events-none" alt="" />
                 ...
                 <img src={gift.icon} className="w-full h-full object-contain pointer-events-none" alt="" />
               )}
            </div>
              ...
              <div className="space-y-8">
                ...
                <div className="bg-black/40 p-6 rounded-3xl border border-white/5 space-y-4">
                   <div className="flex items-center gap-2 mb-2">
                      ...
                      <h4 className="text-xs font-black text-white">أيقونة المتجر</h4>
                   </div>
                   <div className="flex flex-col md:flex-row items-center gap-6">
                      <div className="w-24 h-24 bg-slate-800 rounded-2xl border border-white/10 flex items-center justify-center overflow-hidden shrink-0 relative">
                         <div className="absolute inset-0 z-20 bg-transparent"></div>
                         {editingGift.catalogIcon ? <img src={editingGift.catalogIcon} className="w-full h-full object-contain pointer-events-none" /> : <ImageIcon className="text-slate-700" size={32} />}
                      </div>
                      <div className="flex-1 w-full space-y-3">
                         <div className="relative group">
                            <input 
                               type="text" 
                               value={editingGift.catalogIcon?.substring(0, 15) + '... (Protected)'} 
                               readOnly
                               className="w-full bg-black/40 border border-white/10 rounded-xl p-3 text-[10px] text-slate-500 outline-none cursor-default" 
                            />
                            <ShieldCheck size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-emerald-500 opacity-40" />
                         </div>
                         <div className="space-y-1.5">
                            <p className="text-[8px] text-slate-500 px-2">لإضافة رابط جديد، الصقه هنا:</p>
                            <input 
                               type="text" 
                               placeholder="https://..." 
                               onChange={e => setEditingGift({...editingGift, catalogIcon: e.target.value})}
                               className="w-full bg-black/60 border border-white/10 rounded-xl p-3 text-[10px] text-blue-400 outline-none" 
                            />
                         </div>
                ...
                <div className="bg-black/40 p-6 rounded-3xl border border-white/5 space-y-4">
                   ...
                   <div className="space-y-4">
                      <div className="w-full h-40 bg-slate-800 rounded-2xl border border-white/10 flex items-center justify-center overflow-hidden relative">
                         <div className="absolute inset-0 z-20 bg-transparent"></div>
                      ...
                      <div className="flex flex-col gap-3">
                         <div className="relative">
                            <input 
                               type="text" 
                               value={editingGift.icon?.substring(0, 15) + '... (Protected Content)'} 
                               readOnly
                               className="w-full bg-black/40 border border-white/10 rounded-xl p-3 text-[10px] text-slate-500 outline-none cursor-default" 
                            />
                            <ShieldCheck size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-emerald-500 opacity-40" />
                         ...
                         <div className="space-y-1.5">
                            <p className="text-[8px] text-slate-500 px-2">لإضافة رابط محتوى جديد، الصقه هنا:</p>
                            <input 
                               type="text" 
                               placeholder="الصق رابط MP4 أو GIF هنا..." 
