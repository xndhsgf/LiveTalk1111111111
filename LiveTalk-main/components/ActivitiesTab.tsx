...
import { Gamepad2, Play, Trophy, Sparkles, Star, Globe } from 'lucide-react';
import { GameType, ExternalGame } from '../types';

interface ActivitiesTabProps {
  onOpenGame: (game: GameType) => void;
  externalGames: ExternalGame[];
  onOpenExternalGame: (game: ExternalGame) => void;
}

const ActivitiesTab: React.FC<ActivitiesTabProps> = ({ onOpenGame, externalGames, onOpenExternalGame }) => {
  return (
    <div className="flex flex-col h-full bg-[#030816] p-6 space-y-8 overflow-y-auto scrollbar-hide pb-24" dir="rtl">
      ...
      {externalGames.length > 0 && (
        <div className="space-y-4 pt-4">
           <h3 className="text-sm font-black text-white flex items-center gap-2">
              <Globe size={16} className="text-emerald-400" /> ألعاب عالمية (رصيد حقيقي)
           </h3>
           <div className="grid grid-cols-2 gap-4">
              {externalGames.map(game => (
                <motion.button 
                  key={game.id}
                  whileTap={{ scale: 0.97 }}
                  onClick={() => onOpenExternalGame(game)}
                  className="bg-slate-900/60 border border-white/5 rounded-3xl p-4 flex flex-col items-center gap-3 group relative overflow-hidden"
                >
                   ...
                   <div className="text-center min-w-0 w-full">
                      <h4 className="text-xs font-black text-white truncate">{game.title}</h4>
                      <p className="text-[8px] text-emerald-400 font-bold mt-1">متصل بالرصيد 🪙</p>
                   </div>
                </motion.button>
              ))}
           </div>
