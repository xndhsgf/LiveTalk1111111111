...
import { Home, User as UserIcon, Plus, Bell, Crown, Gem, Settings, ChevronRight, Edit3, Share2, LogOut, Shield, Database, ShoppingBag, Camera, Trophy, Flame, Sparkles, UserX, Star, ShieldCheck, MapPin, Download, Smartphone, MessageCircle, Languages, Smartphone as MobileIcon, Wallet, Medal, Lock, AlertCircle, Key, X, Zap, BadgeCheck, ChevronLeft, Award, Coins, Users, UserPlus, Eye, Heart, Gamepad2, UserCheck, Search, RefreshCw, IdCard } from 'lucide-react';
...
import SpecialIDShopModal from './components/SpecialIDShopModal';
...
import { Room, User, VIPPackage, UserLevel, Gift, StoreItem, GameSettings, GlobalAnnouncement, LuckyBag, GameType, LuckyMultiplier, Banner, ExternalGame, SpecialIDItem } from './types';
...
export default function App() {
      ...
      if (firebaseUser) {
        const unsubUserDoc = onSnapshot(doc(db, 'users', firebaseUser.uid), (docSnap) => {
          if (docSnap.exists()) {
            ...
            let vipLevel = Number(data.vipLevel || 0);
            let frame = data.frame || null;
            
            if (isVip && data.vipExpiry) {
              const expiry = data.vipExpiry.toMillis();
              if (Date.now() > expiry) {
                isVip = false;
                vipLevel = 0;
                frame = null;
                updateDoc(doc(db, 'users', firebaseUser.uid), {
                  ...
                  vipLevel: 0,
                  frame: null
                }).catch(e => console.error("VIP Expiry Update Failed:", e));
              }
            }
  ...
  useEffect(() => {
    ...
    const unsubExtGames = onSnapshot(collection(db, 'external_games'), (snapshot) => {
      setExternalGames(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as ExternalGame)));
    });
    ...
    return () => { unsubIdentity(); unsubBanners(); unsubExtGames(); unsubGameSettings(); unsubUsersList(); unsubRooms(); unsubGifts(); unsubStore(); unsubVIP(); unsubAnnouncements(); };
  }, []);
  ...
  const handleSpecialIDPurchase = async (sid: SpecialIDItem) => {
    if (!user) return;
    if (user.coins < sid.price) return alert('رصيدك لا يكفي لشراء هذا الآيدي ❌');
    
    if (!confirm(`هل أنت متأكد من شراء الآيدي المميز (${sid.customId}) مقابل ${sid.price.toLocaleString()} كوينز؟`)) return;

    try {
      const batch = writeBatch(db);
      // 1. تحديث المستخدم
      batch.update(doc(db, 'users', user.id), {
        customId: sid.customId,
        badge: sid.badgeUrl,
        isSpecialId: true,
      ...
      handleUpdateUser({
        ...
        badge: sid.badgeUrl,
        isSpecialId: true,
        coins: user.coins - sid.price,
        wealth: Number(user.wealth || 0) + sid.price
      });
      
      alert(`مبروك! لقد حصلت على الآيدي الملكي الجديد: ${sid.customId} ✨`);
      setShowSpecialIDModal(false);
    } catch (e) {
      alert('فشلت عملية الشراء، يرجى المحاولة لاحقاً');
    }
  };
  ...
  return (
    <div className="h-[100dvh] w-full bg-[#030816] text-white relative md:max-w-md mx-auto shadow-2xl overflow-hidden flex flex-col font-cairo">
      ...
      <div className="flex-1 overflow-y-auto pb-24 scrollbar-hide">
        {activeTab === 'home' && (
           <div className="mt-2 space-y-3 px-4 relative">
              ...
              <div className="relative w-full h-28 rounded-2xl overflow-hidden bg-slate-800 border border-white/5 shadow-lg">
                <AnimatePresence mode="wait">
                  {banners.length > 0 ? (
                    <motion.a
                      key={banners[currentBannerIdx].id}
                      href={banners[currentBannerIdx].link || '#'}
                      target="_blank"
                    ...
                    >
                      <img src={banners[currentBannerIdx].imageUrl} className="w-full h-full object-cover" alt={banners[currentBannerIdx].title} />
                    </motion.a>
                  ) : (
                    appBanner && <img src={appBanner} className="w-full h-full object-cover" />
                  )}
                </AnimatePresence>
                {banners.length > 1 && (
                  <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1 z-10">
                    {banners.map((_, i) => (
                      <div key={i} className={`w-1.5 h-1.5 rounded-full transition-all ${i === currentBannerIdx ? 'bg-amber-500 w-3' : 'bg-white/40'}`} />
                    ))}
                  </div>
                )}
              </div>
        ...
        {activeTab === 'rank' && <ActivitiesTab onOpenGame={setActiveGame} externalGames={externalGames} onOpenExternalGame={setSelectedExternalGame} />}
        {activeTab === 'profile' && (
          <div className="flex flex-col bg-[#030816] min-h-full" dir="rtl">
            ...
            <div className="grid grid-cols-3 gap-3 px-4 mt-5">
              <button onClick={() => setShowWalletModal(true)} className="h-20 rounded-2xl bg-gradient-to-r from-blue-400 to-cyan-500 flex flex-col items-center justify-center shadow-lg"><Wallet size={20}/><span className="font-black text-[11px] mt-1 uppercase">Wallet</span></button>
              <button onClick={() => setShowSpecialIDModal(true)} className="h-20 rounded-2xl bg-gradient-to-r from-amber-400 to-orange-600 flex flex-col items-center justify-center shadow-lg"><IdCard size={20}/><span className="font-black text-[11px] mt-1 uppercase text-black">Shop ID</span></button>
              <button onClick={() => setShowVIPModal(true)} className="h-20 rounded-2xl bg-gradient-to-r from-blue-600 to-blue-800 flex flex-col items-center justify-center shadow-lg"><Crown size={20}/><span className="font-black text-[11px] mt-1 uppercase">VIP</span></button>
            </div>
      ...
      {showSpecialIDModal && <SpecialIDShopModal isOpen={showSpecialIDModal} onClose={() => setShowSpecialIDModal(false)} currentUser={user} onBuy={handleSpecialIDPurchase} />}
      ...
      {selectedExternalGame && <ExternalGameModal isOpen={!!selectedExternalGame} onClose={() => setSelectedExternalGame(null)} game={selectedExternalGame} currentUser={user} onUpdateUser={handleUpdateUser} />}
